#!/bin/bash
# Celery 容器化重启脚本
# 用法: ./start_celery.sh [--restart|-r] [--force-recreate|-f] [--logs|-l]
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "选项:"
            echo "  --restart, -r        自动重启容器（无需确认）"
            echo "  --force-recreate, -f 强制重建容器（包含 --restart）"
